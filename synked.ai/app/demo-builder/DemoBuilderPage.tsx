...
export function DemoBuilderPage() {
  ...
  const [savedProjects, setSavedProjects] = useState<
    Array<{ id: string; name: string; updatedAt: string; config: unknown }>
  >([]);
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      const saved = localStorage.getItem("synked-builder-draft");
      if (saved) {
        try {
          const { nodes, connections } = JSON.parse(saved);
          if (nodes?.length) {
            setPlaced(nodes);
  ...
  useEffect(() => {
    if (placed.length > 0) {
      localStorage.setItem(
        "synked-builder-draft",
        JSON.stringify({ nodes: placed, connections: conns }),
      );
    }
  }, [placed, conns]);

  // Auto-save to server (debounced 30s) when project is loaded
  useEffect(() => {
    if (!projectId || placed.length === 0) return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [placed, conns, projectId]);

  const fetchProjects = useCallback(async () => {
    try {
      const res = await fetch("/api/user/projects?limit=20");
      if (res.ok) {
  ...
  async function loadProject(id: string) {
    try {
      ...
      if (project.config) {
        ...
        setPlaced(config.nodes || []);
        setConns(config.connections || []);
      }
    } catch {
      /* ignore */
    }
  ...
  async function saveToServer(showFeedback = true) {
    if (saving) return;
    const config = { nodes: placed, connections: conns };
  ...
  async function deleteProject(id: string) {
    try {
      ...
      if (projectId === id) {
        ...
        setProjectName("");
        reset();
        window.history.replaceState(null, "", "/demo-builder");
      }
    } catch {
      /* ignore */
    }
            ...
            >
              ...
              {projectId && (
                <input
                  ...
                  style={{
                    ...
                    border: "none",
                    borderBottom: "1px solid var(--color-night-bord)",
                    color: "var(--color-bone)",
                    fontSize: "0.8rem",
                    fontFamily: "var(--font-mono)",
                    padding: "0.3rem 0",
              ...
              >
                ...
                <button
                  ...
                  disabled={placed.length === 0 || saving}
                  style={{
                    background: "none",
                    border: "1px solid var(--color-night-bord)",
                    color:
                      saveStatus === "saved"
                        ? "var(--color-pistachio)"
                        : "var(--color-night-mono)",
                    padding: "0.48rem 0.95rem",
                    fontSize: "0.68rem",
                    fontFamily: "var(--font-mono)",
                    borderRadius: 2,
                    cursor:
                      placed.length === 0 || saving ? "not-allowed" : "pointer",
                    transition: "border-color 0.2s, color 0.2s",
          ...
          {showProjectsPanel && (
            ...
            >
              ...
              {savedProjects.length === 0 ? (
                <p
                  style={{ fontSize: "0.8rem", color: "rgba(235,220,200,0.4)" }}
                >
